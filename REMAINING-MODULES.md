...
**Built 🛠️ (backend, in Docker, on feature branches → PR):** Module 0 — Platform Foundation; **Module 1 — Onboarding** (218 tests); **Module 2 — House & Occupancy** (372 tests); **Module 3 — Vault** (521 tests); **Module 4 — Finance** (full suite 689); **Module 5 — Complaints** (`docs/implemented/complaints.md`; full suite 857) — house-scoped complaints with the status workflow, Vault-stored images, worker auto-archive, and the new in-process event dispatcher (`app/common/events.py`) that Notifications will subscribe to; **Module 6 — Notice Board** (`docs/implemented/notice-board.md`; full suite 1010) — society-wide broadcast (draft→published→withdrawn, edit/pin/query-time-expiry), rich-text bodies + titles **sanitized** server-side (new `nh3` dep + `app/common/html_sanitizer.py`), unlimited Vault attachments, admin read receipts (current-owner denominator), emits `notice_posted` + `mark_read_for` to the dispatcher (skeleton — no-op until Notifications subscribes). Next module to build: **Notifications** (Module 7, which wires the dispatcher — subscribing to the `complaint.*` and `notice_posted` events already emitted by Complaints + Notice Board).
