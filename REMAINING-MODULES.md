...
**Built 🛠️ (backend, in Docker, on feature branches → PR):** Module 0 — Platform Foundation; **Module 1 — Onboarding** (218 tests); **Module 2 — House & Occupancy** (372 tests); **Module 3 — Vault** (521 tests); **Module 4 — Finance** (full suite 689); **Module 5 — Complaints** (`docs/implemented/complaints.md`; full suite 857) — house-scoped complaints with the status workflow, Vault-stored images, worker auto-archive, and the new in-process event dispatcher (`app/common/events.py`) that Notifications will subscribe to. Next module to build: **Notice Board** (Module 6), then **Notifications** (Module 7, which wires the dispatcher).
