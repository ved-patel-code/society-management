...
| Finance (due reminders) | Notifications + worker | dues service callable standalone; Finance exposes `outstanding_dues` + `maintenance_due_day` (built); reminder rule hosted by Notifications | ✅ wired (Notifications built) — daily worker scan, consolidated `maintenance_due` per owing house |
| Notifications (maintenance due) | Finance | consumes `outstanding_dues` + `maintenance_due_day` (both provided by built Finance); `dedupe_key = dues:{house}:{day}` | ✅ wired (Notifications built) — per-house-per-day idempotent, per-recipient suffix |
