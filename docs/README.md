...
>
> **Built so far:** Module 0 — Platform Foundation ([implemented/platform-foundation.md](implemented/platform-foundation.md)) · Module 1 — Onboarding ([implemented/onboarding.md](implemented/onboarding.md)). Remaining modules build one at a time on feature branches, in the order below.
...
- [modules/house-occupancy.md](modules/house-occupancy.md) — ✅ designed. House status lifecycle, owner/tenant occupancy, ID proof (optional), status filters.
- [modules/vault.md](modules/vault.md) — ✅ designed. File-manager document storage (MinIO), house-centric folders, Trash, 5 GB default limit, admin-only.
